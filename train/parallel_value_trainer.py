...
from value_function import ParallelValueFunction
...
def get_dataset(dataset_name, tokenizer=None):
    ...
    elif dataset_name == 'numina-v1-steps':
        ds = datasets.load_dataset('violetxi/Value-v1-NUMINA-V1-Steps-Merged-561-step-len-filtered')
    elif dataset_name == 'numina-v2-blocks-980':
        ds = datasets.load_dataset('RLAIF/Value-v2-NUMINA-V2-Blocks-Merged-980-problems-step-len-filtered')
    elif dataset_name == 'numina-v2-blocks-1999':
        ds = datasets.load_dataset('RLAIF/Value-v2-NUMINA-V2-Blocks-Merged-1999-problems-step-len-filtered')
...
def main(args: DictConfig):
    ...
    model = ParallelValueFunction(**value_args)
    ...
    def data_collator(batch):
        ...
        for i, solution_steps in enumerate(batch_dict['solution_steps']):
            # replace STEP_TOKEN in problem text with newlines
            problem = batch_dict['problem'][i].replace(STEP_TOKEN, "\n").strip()
            solution_str = f"Question: {problem}\n\nSolution: "
            assert len(solution_steps) == len(batch_dict['rtgs'][i]) == len(batch_dict['undiscounted_rtgs'][i]), \
                f"Solution steps and RTGs must have the same length: {len(solution_steps)} != {len(batch_dict['rtgs'][i])}"            
        ...
        output_batch['rtgs'] = rtg
        output_batch['undiscounted_rtgs'] = undiscounted_rtg
    ...
    if args.balance_dataset:    # balancing positive and negative examples in the dataset
        ...
        ds_train = ds['train']
        ds_train_positive = ds_train.filter(filter_fn_positive, batched=True, num_proc=os.cpu_count())
        ds_train_negative = ds_train.filter(filter_fn_negative, batched=True, num_proc=os.cpu_count())
        ds_test = ds['test']
        ds_test_positive = ds_test.filter(filter_fn_positive, batched=True, num_proc=os.cpu_count())
        ds_test_negative = ds_test.filter(filter_fn_negative, batched=True, num_proc=os.cpu_count())        
        ...
        def itr_fn(dataloader1, dataloader2, tokenizer, num_epochs_steps):
            ...
            while step < num_epochs_steps:
                for x, y in zip(dataloader1, dataloader2):               
                    ...
                    new_batch = dict()                    
                    shuffle_indices = torch.randperm(len(x['rtgs']) + len(y['rtgs']))                    
    ...
    for epoch_num in range(args.epochs):
        for i, batch in tqdm.tqdm(enumerate(train_dataloader), total=num_epochs_steps, desc="parallel_value_trainer"):
            ...
            if args.debug:
                if total_steps % args.save_freq == 0 and total_steps > 0:
                    ...
                    model_id = f"{args.wandb_project}_{args.wandb_run_name}_checkpoint{total_steps}"                    
                    model.save(full_output_dir, model_id=model_id) 
            else:
                if i % args.save_freq == 0 and i > 0:
                    full_output_dir = os.path.join(args.output_dir, args.wandb_project, args.wandb_run_name, f"checkpoint_{i}")
                    os.makedirs(full_output_dir, exist_ok=True)
                    model_id = f"{args.wandb_project}_{args.wandb_run_name}_checkpoint{i}"                    
                    model.save(full_output_dir, model_id=model_id)
            ...
            if i % args.test_freq == 0:
                with torch.no_grad():
                    ...
                    for _ in tqdm.tqdm(range(args.num_test_batches), total=args.num_test_batches, desc="Testing"):                        
                        ...
                        rewards = torch.sigmoid(logits) * test_prm_mask
                        
                        p_1, p_2 = test_reward, rewards
                        test_loss = stable_loss(p_1, p_2)                        

                        masked_orm_loss = test_loss[test_orm_mask]
                        masked_prm_loss = test_loss[test_prm_mask]
                        
                        masked_orm_reward = rewards[test_orm_mask]
                        masked_prm_reward = rewards[test_prm_mask]
                        
                        masked_orm_target_reward = test_reward[test_orm_mask]
                        masked_prm_target_reward = test_reward[test_prm_mask]                        
                        
                        mse = (rewards - test_reward) ** 2
                        masked_orm_mse = mse[test_orm_mask]
                        masked_prm_mse = mse[test_prm_mask]
                        
                        rounded = torch.round(rewards)
                        rounded_target = torch.round(test_reward)
                        test_accuracy = (rounded == rounded_target).float()
                        
                        masked_orm_accuracy = test_accuracy[test_orm_mask]
                        masked_prm_accuracy = test_accuracy[test_prm_mask]
                        
                        all_stacked['test/orm_loss'].append(masked_orm_loss.float().mean())
                        all_stacked['test/prm_loss'].append(masked_prm_loss.float().mean())
                        all_stacked['test/orm_reward'].append(masked_orm_reward.float().mean())
                        all_stacked['test/prm_reward'].append(masked_prm_reward.float().mean())
                        all_stacked['test/orm_target_reward'].append(masked_orm_target_reward.float().mean())
                        all_stacked['test/prm_target_reward'].append(masked_prm_target_reward.float().mean())
                        all_stacked['test/orm_accuracy'].append(masked_orm_accuracy.float().mean())
                        all_stacked['test/prm_accuracy'].append(masked_prm_accuracy.float().mean())
                        all_stacked['test/orm_mse'].append(masked_orm_mse.float().mean())
                        all_stacked['test/prm_mse'].append(masked_prm_mse.float().mean())
                        
                        # now look at positive and negative examples
                        final_reward = (test_reward * test_orm_mask).sum(axis=1)
                        pos_mask = (final_reward > 0.5).unsqueeze(-1).expand_as(rewards)
                        neg_mask = (final_reward <= 0.5).unsqueeze(-1).expand_as(rewards)
                        
                        pos_orm_mask = pos_mask & test_orm_mask
                        neg_orm_mask = neg_mask & test_orm_mask
                        
                        pos_orm_accuracy = test_accuracy[pos_orm_mask]
                        neg_orm_accuracy = test_accuracy[neg_orm_mask]
                        
                        pos_orm_mse = mse[pos_orm_mask]
                        neg_orm_mse = mse[neg_orm_mask]
                        
                        pos_prm_mask = pos_mask & test_prm_mask
                        neg_prm_mask = neg_mask & test_prm_mask
                        
                        pos_prm_accuracy = test_accuracy[pos_prm_mask]
                        neg_prm_accuracy = test_accuracy[neg_prm_mask]
                        
                        pos_prm_mse = mse[pos_prm_mask]
                        neg_prm_mse = mse[neg_prm_mask]
                        
                        all_stacked['test/pos_orm_accuracy'].append(pos_orm_accuracy.float().mean())
                        all_stacked['test/neg_orm_accuracy'].append(neg_orm_accuracy.float().mean())
                        all_stacked['test/pos_orm_mse'].append(pos_orm_mse.float().mean())
                        all_stacked['test/neg_orm_mse'].append(neg_orm_mse.float().mean())
                        all_stacked['test/pos_prm_accuracy'].append(pos_prm_accuracy.float().mean())
                        all_stacked['test/neg_prm_accuracy'].append(neg_prm_accuracy.float().mean())
                        all_stacked['test/pos_prm_mse'].append(pos_prm_mse.float().mean())
                        all_stacked['test/neg_prm_mse'].append(neg_prm_mse.float().mean())
            ...
            with accelerator.accumulate(model):
                ...
                train_loss = stable_loss(p_1, p_2) * train_prm_mask
                final_train_loss = train_loss.sum() / (train_prm_mask.sum() + 1e-9)

                accelerator.backward(final_train_loss)               
                optimizer.step()
                scheduler.step()                
                optimizer.zero_grad()
                
                with torch.no_grad():
                    masked_orm_loss = (train_loss * train_orm_mask).sum(axis=1) / (train_orm_mask.sum(axis=1) + 1e-9)
                    masked_prm_loss = (train_loss * train_prm_mask).sum(axis=1) / (train_prm_mask.sum(axis=1) + 1e-9)
                    
                    masked_orm_reward = (rewards * train_orm_mask).sum(axis=1) / (train_orm_mask.sum(axis=1) + 1e-9)
                    masked_prm_reward = (rewards * train_prm_mask).sum(axis=1) / (train_prm_mask.sum(axis=1) + 1e-9)
                    
                    masked_orm_target_reward = (train_reward * train_orm_mask).sum(axis=1) / (train_orm_mask.sum(axis=1) + 1e-9)
                    masked_prm_target_reward = (train_reward * train_prm_mask).sum(axis=1) / (train_prm_mask.sum(axis=1) + 1e-9)
                    
                    rounded = torch.round(rewards)
                    rounded_target = torch.round(train_reward)
                    train_accuracy = (rounded == rounded_target).float()
                    
                    masked_orm_accuracy = train_accuracy[train_orm_mask]
                    masked_prm_accuracy = train_accuracy[train_prm_mask]
                    
                    mse = (rewards - train_reward) ** 2
                    masked_orm_mse = mse[train_orm_mask]
                    masked_prm_mse = mse[train_prm_mask]
                    
                    # now look at positive and negative examples
                    final_reward = (train_reward * train_orm_mask).sum(axis=1)
                    pos_mask = (final_reward > 0.5).unsqueeze(-1).expand_as(rewards)
                    neg_mask = (final_reward <= 0.5).unsqueeze(-1).expand_as(rewards)
                    
                    pos_orm_mask = pos_mask & train_orm_mask
                    neg_orm_mask = neg_mask & train_orm_mask
                    
                    pos_orm_accuracy = train_accuracy[pos_orm_mask]
                    neg_orm_accuracy = train_accuracy[neg_orm_mask]

                    pos_orm_mse = mse[pos_orm_mask]
                    neg_orm_mse = mse[neg_orm_mask]
                    
                    pos_prm_mask = pos_mask & train_prm_mask
                    neg_prm_mask = neg_mask & train_prm_mask
                    
                    pos_prm_accuracy = train_accuracy[pos_prm_mask]
                    neg_prm_accuracy = train_accuracy[neg_prm_mask]
                    
                    pos_prm_mse = mse[pos_prm_mask]
                    neg_prm_mse = mse[neg_prm_mask]                    
                
                lr = optimizer.param_groups[0]['lr']
                accelerator.print(f"Step {i} | Train Loss: {train_loss.float().mean()} | Train PRM Accuracy: {masked_prm_accuracy.float().mean()} | Train ORM Accuracy: {masked_orm_accuracy.float().mean()}")
                accelerator.log({
                    'train/lr': lr,
                    'train/prm_loss': masked_prm_loss.float().mean(),
                    'train/orm_loss': masked_orm_loss.float().mean(),
                    'train/prm_reward': masked_prm_reward.float().mean(),
                    'train/orm_reward': masked_orm_reward.float().mean(),
                    'train/prm_target_reward': masked_prm_target_reward.float().mean(),
                    'train/orm_target_reward': masked_orm_target_reward.float().mean(),
                    'train/prm_accuracy': masked_prm_accuracy.float().mean(),
                    'train/orm_accuracy': masked_orm_accuracy.float().mean(),
                    'train/prm_mse': masked_prm_mse.float().mean(),
                    'train/orm_mse': masked_orm_mse.float().mean(),
                    'train/pos_orm_accuracy': pos_orm_accuracy.float().mean(),
                    'train/neg_orm_accuracy': neg_orm_accuracy.float().mean(),
                    'train/pos_orm_mse': pos_orm_mse.float().mean(),
                    'train/neg_orm_mse': neg_orm_mse.float().mean(),
                    'train/pos_prm_accuracy': pos_prm_accuracy.float().mean(),
                    'train/neg_prm_accuracy': neg_prm_accuracy.float().mean(),
                    'train/pos_prm_mse': pos_prm_mse.float().mean(),
                    'train/neg_prm_mse': neg_prm_mse.float().mean(),
        ...
        if not args.debug:
            ...
            epoch_model_id = f"{args.hf_organization}/{args.wandb_project}_{args.wandb_run_name}_epoch{epoch_num}"            
            model.save(full_output_dir, model_id=epoch_model_id)
