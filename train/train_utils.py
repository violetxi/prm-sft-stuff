...
import typing
from transformers import DataCollatorForLanguageModeling, PreTrainedModel, PreTrainedTokenizerBase, TrainerCallback, BatchEncoding
import warnings
import os
import importlib
...
def cat_batch_encoding(batch: List[BatchEncoding], dim: int = -1):
    return BatchEncoding({
        k: torch.cat([b[k] for b in batch], dim=dim)
        for k in batch[0].keys()
    })


def semantic_concatenate(str1: str, str2: str):
    return f"Context: {str1} \nResponse: {str2}"

def cat_batch_encoding_semantic(batch_list: List[BatchEncoding], tokenizer: AutoTokenizer, tok_kwargs: Dict[str, Any] = {}):
    # decode the token ids to strings
    decoded_batch = [tokenizer.batch_decode(b["input_ids"]) for b in batch_list]
    # combine the strings
    assert len(decoded_batch) == 2, f"Expected 2 batch encodings, got {len(decoded_batch)}"
    
    all_cats = []
    for s1, s2 in zip(decoded_batch[0], decoded_batch[1]):
        all_cats.append(semantic_concatenate(s1, s2))
    all_cats = tokenizer(all_cats, **tok_kwargs)
...
def ema_model_update(model, ema_model, decay):
    """
    Update the EMA model with the model parameters.
    """
